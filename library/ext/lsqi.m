function [x_alpha,lambda,maxIterReached] = lsqi(U,s,V,b,alpha,x_0)
...
% Modified 7/31/12 as part of the Signal Space CoSaMP Toolbox by Mark 
% Davenport, Deanna Needell, Michael Wakin: We have modified lsqi.m to 
% return a flag if it does not converge (rather than printing a warning). 
...
if (ps == 1) 
  ...
  for k=1:la
    if (norm_x_unreg <= alpha(k))
      ...
      [lambda(k),maxIterReachedThisTime] = heb_new(lambda_0,alpha(k),s,beta,omega);
      maxIterReached = maxIterReachedThisTime | maxIterReached;
  ...
  for k=1:la
    if (norm_Lx_unreg <= alpha(k))
      ...
      [lambda(k),maxIterReachedThisTime] = heb_new(lambda_0,alpha(k),s,beta(1:p),omega);
      maxIterReached = maxIterReachedThisTime | maxIterReached;
...
function [lambda,maxIterReached] = heb_new(lambda_0,alpha,s,beta,omega)
...
if (abs(step) > thr*lambda), 
    % disp('Max. number of iterations reached in Hebden-Newton iteration of lsqi');
    maxIterReached = true;
else
    maxIterReached = false; 
end