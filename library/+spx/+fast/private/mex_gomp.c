...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray*prhs[])
{
    ...
    if(verbose){
        mexPrintf("M: %d, N:%d, S: %d, K: %d, L: %d, eps: %e, sparse: %d, verbose: %d\n",
         M, N, S, K, L, eps, sparse_output, verbose);
    }
    switch(ls_method){
        case LS_CHOL:
             A_OUT = gomp_chol(m_dict, m_x, M, N, S, K, L, eps, sparse_output, verbose);
            break;
        case LS_LS:
            A_OUT = gomp_ls(m_dict, m_x, M, N, S, K, L, eps, sparse_output, verbose);
            break;
        default:
