...
% Skeleton for any operator in CSF
     ...
     methods
        ...
        function result = transpose(self)
            [m, ~] = self.size();
            result = spx.dict.MatrixOperator(self.apply_transpose(speye(m, m)));
        end

        function result = ctranspose(self)
            [m, ~] = self.size();
            result = spx.dict.MatrixOperator(self.apply_ctranspose(speye(m, m)));
        end
        ...
        function result = columns_operator(self, columns)
            ...
            post = sparse(is, js, vals, n, nc);
            tmp = self.apply(post);
            result  = spx.dict.MatrixOperator(tmp);
        end
