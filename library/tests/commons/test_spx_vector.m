...
function test_reverse(testCase)
    v1 = [1 2 3];
    v2 = [3 2 1];
    verifyEqual(testCase, v1, spx.commons.vector.reverse(v2));
end

function test_reshape_as_row(testCase)
    x = [1 2 3];
    y = spx.commons.vector.reshape_as_row_vec(x);
...
function test_reshape_as_col(testCase)
    x = [1 2 3];
    y = spx.commons.vector.reshape_as_col_vec(x);
    verifyEqual(testCase, x', y);
    x = x';
    y = spx.commons.vector.reshape_as_col_vec(x);
    verifyEqual(testCase, x, y);
...
function test_reshape_as_proto(testCase)
    x = [1 2 3];
    p = [1 2 3 4]';
    y = spx.commons.vector.reshape_as_prototype(x, p);
    verifyEqual(testCase, x', y);
    p = p';
    y = spx.commons.vector.reshape_as_prototype(x, p);
    verifyEqual(testCase, x, y);
...
function test_shift_r(testCase)
    x = [1 2 3];
    y = spx.commons.vector.shift_r(x);
...
function test_shift_rc(testCase)
    x = [1 2 3];
    y = spx.commons.vector.shift_rc(x);
...
function test_shift_l(testCase)
    x = [1 2 3];
    y = spx.commons.vector.shift_l(x);
...
function test_shift_lc(testCase)
    x = [1 2 3];
    y = spx.commons.vector.shift_lc(x);
...
function test_shift_rn(testCase)
    x = [1 2 3 4];
    y = spx.commons.vector.shift_rn(x, 2);
...
function test_shift_rcn(testCase)
    x = [1 2 3 4];
    y = spx.commons.vector.shift_rcn(x, 2);
...
function test_shift_ln(testCase)
    x = [1 2 3 4];
    y = spx.commons.vector.shift_ln(x, 2);
...
function test_shift_lcn(testCase)
    x = [1 2 3 4];
    y = spx.commons.vector.shift_lcn(x, 2);
    verifyEqual(testCase, y, [3 4 1 2]);
    x = x';
    y = spx.commons.vector.shift_lcn(x, 2);
    verifyEqual(testCase, y, [3 4 1 2]');
...
function test_repeat_at_end(testCase)
    x  = 1:4;
    y = spx.commons.vector.repeat_vector_at_end(x, 2);
    verifyEqual(testCase, y, [1:4 1:2]);
    y = spx.commons.vector.repeat_vector_at_end(x, 4);
    verifyEqual(testCase, y, [1:4 1:4]);
    y = spx.commons.vector.repeat_vector_at_end(x, 6);
    verifyEqual(testCase, y, [1:4 1:4 1:2]);
end

function test_repeat_at_start(testCase)
    x  = 1:4;
    y = spx.commons.vector.repeat_vector_at_start(x, 2);
    verifyEqual(testCase, y, [3:4 1:4]);
    y = spx.commons.vector.repeat_vector_at_start(x, 4);
    verifyEqual(testCase, y, [1:4 1:4]);
    y = spx.commons.vector.repeat_vector_at_start(x, 6);
