...
if perform_ssc_omp 
    ...
    omp_solver = spx.cluster.ssc.SSC_OMP(X, K, ns);
    ...
    colormap gray;
    imagesc(abs(omp_solver.Representation));
end



if perform_ssc_batch_omp 
    tstart = tic; 
    % Application of Sparse subspace clustering
    batch_omp_solver = spx.cluster.ssc.SSC_BATCH_OMP(X, K, ns);
    result_batch_omp = batch_omp_solver.solve();
    elapsed_time = toc(tstart);
    fprintf('SSC Batch OMP time spent: %.2f seconds\n', elapsed_time);
    ...
    mf.new_figure('SSC-Batch-OMP Coefficients');
    colormap gray;
    imagesc(abs(batch_omp_solver.Representation));
end
